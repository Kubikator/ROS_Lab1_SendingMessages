...
void laserCallback(const sensor_msgs::LaserScan& scan)
{
    ...
    for (std::size_t i = 0; i < scan.ranges.size(); ++i) 
    {
        ...
        tf::Vector3 v = scanTransform(tf::Vector3(scan.ranges[i]*cos(angle), scan.ranges[i]*sin(angle), 0));
        ...
        for (std::size_t j = 0; j*map_msg.info.resolution < scan.ranges[i]; ++j)
        {
        	...
        	tf::Vector3 v_j = scanTransform(tf::Vector3(j*map_msg.info.resolution*cos(angle), j*map_msg.info.resolution*sin(angle), 0));
